...
| #1053 | 单元格框线设置 | `已解决` | 已补每个 td 独立 `borderColor` / `borderWidth`，公开命令为 `executeTableTdBorderColor()` / `executeTableTdBorderWidth()`；[issue-1053-table-cell-border.cy.ts](/D:/canvas-editor/cypress/e2e/issues/issue-1053-table-cell-border.cy.ts) 与 [table-pagination-border.cy.ts](/D:/canvas-editor/cypress/e2e/menus/table-pagination-border.cy.ts) 覆盖整表边框类型/颜色、单元格单边框设置/取消、单元格边框样式数据往返和分页 fragment 顶边框渲染。 | 本地已验证，后续仅保留回归。 |
