...
| --- | --- | --- | --- | --- |
| #1404 | 插入表格的时候当colgroup未传入时，默认使用编辑器宽度平分 | `需人工验证` | 本地存在 `colgroup` 相关表格实现与宽度逻辑，如 [TableOperate.ts](/D:/canvas-editor/src/editor/core/draw/particle/table/TableOperate.ts) 与 [option.md](/D:/canvas-editor/docs/guide/option.md) 的 `defaultColMinWidth`，但未见直接对应“未传 colgroup 默认平分”的回归说明。 | 先补一个“插入无 colgroup 表格”的本地用例，再决定是否推进。 |
...
| #1385 | 官网的demo中 在文本、列举控件中 插入下划线 显示异常 | `需人工验证` | 本地下划线渲染链明显重构过，见 [RowRenderer.ts](/D:/canvas-editor/src/editor/core/draw/render/RowRenderer.ts) 与 [Underline.ts](/D:/canvas-editor/src/editor/core/draw/richtext/Underline.ts)，但未见对应 issue 场景测试。 | 用 demo 场景专项回归。 |
...
| #440 | 文档列表内容内无法取消或者增加子列表 | `需人工验证` | 本地列表能力存在，但 issue 是交互 bug，未见对应测试。 | 按列表内增删子列表场景人工验证。 |
| #438 | 可否增加文档处理  没有标尺值功能 | `未见证据` | 未见无标尺值处理能力的明确说明。 | 作为未实现需求列入 backlog。 |
| #425 | 文本控件内容中，再插入控件，页面{}显示有问题。 | `需人工验证` | 控件与嵌套主线复杂，未见直接 issue 级回归。 | 用“文本控件内再插控件”场景专项验证。 |
...
| #41 | 表格分页 | `部分解决` | 本地已有大规模分页表格重构、文档、测试与 README 主线说明，见 [table-refactor-plan.md](/D:/canvas-editor/docs/guide/table-refactor-plan.md)、[table-refactor-tasks.md](/D:/canvas-editor/docs/guide/table-refactor-tasks.md)、[README.md](/D:/canvas-editor/README.md)。但 issue 仍 open，说明主线仍在持续推进。 | 视为“核心主线已落地但未完全收官”，必须保留持续回归。 |
