...
8. 已完成 P0 垂直切片：新增 `WorkerRenderScheduler`，实现每页最新 job、超时、过期丢弃和 Canvas2D 回退。
...
12. 已完成核心六第一批：补 worker 单并发上限、同页主动取消、队列上限、队列丢弃 回退 和连续失败熔断。
...
15. 已完成核心七第二批：补 WebGL context lost 后 Canvas2D 回退 回归，补 DOM / SVG block 页面卸载清理、重挂和 bitmap cache 命中后的 host 重放。
16. 已完成核心八第一批：搜索活动态和激活控件页在调度前即归为交互页，base 保持 `sync` + `canvas-2d-render`，不进入 worker，也不会被 worker bitmap 覆盖。
17. 已完成核心八第二批：补 worker 不支持命令、超时、队列丢弃和连续失败熔断的浏览器回归；回退 后不产生 backend miss，熔断后 OffscreenCanvas 不再匹配任务。
18. 已完成核心八第三批：补 WebGL 图片导出和 DOM / SVG block 导出 回退，导出期间不命中 `webgl` / `svg-dom`，统一通过 Canvas2D export surface 固化输出。
19. 已完成核心八第四批：补真实门诊模板 100 / 500 页 worker 观测，验证 worker 命中率、bitmap 写入来源、回退 原因、熔断状态和队列清空均可观测。
