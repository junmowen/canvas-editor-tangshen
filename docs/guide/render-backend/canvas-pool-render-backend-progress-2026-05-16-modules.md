...
34. `DrawRenderFacadeService` 已继续拆分：局部 patch 后的 base / overlay bitmap 失效、表格迁移旧页清理和额外可见页重绘抽到 `DrawRenderSurfaceInvalidator`；chunk patch / line fallback 决策和 debug 日志抽到 `TypingPatchCoordinator`；表格输入的 td 子 chunk dirty 命中和当前逻辑表页码解析抽到 `TableTypingRenderHelper`；通用 image observer 清理、cursor 恢复、pageCount 更新、render pipeline 调用、history 提交和 postRenderEffects 调度抽到 `DrawRenderFinalizeService`。门面当前保留三条主流程编排：普通 render、主文档 typing、表格 typing。
