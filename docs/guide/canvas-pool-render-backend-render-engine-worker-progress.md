...
5. `canvas-render-backend.cy.ts` 已增加实验 engine 故障注入用例，验证失败后能回退到 Canvas2D、页面像素仍非空、miss 为 0，并记录失败 engine 与 fallback 目标。
...
12. `Draw.getRenderBackendStats().workerRender` 已暴露 worker submit、success、fallback、stale discard、compose reject、timeout 和 pending job 统计。
