...
这份文档从 [upstream-open-issues-solo-full-table.md](./upstream-open-issues-solo-full-table.md) 中只提取当前状态为 `未验证` 的条目。
...
8. `#1190` 已补普通文本 Tab 缩进专项样例并通过：`issue-1190-tab-indent-text.cy.ts` / `issue-440-list-sublevel.cy.ts`，覆盖普通文本插入带样式 `TAB` 元素和列表 Tab/Shift+Tab 层级调整。
...
12. `#1053` 已补单元格边框设置实现与专项样例并通过：`issue-1053-table-cell-border.cy.ts` / `table-pagination-border.cy.ts`，覆盖整表边框类型/颜色、单元格单边框设置/取消、仅设置 td 边框颜色/宽度时自动显示四边、每个 td 独立 `borderColor` / `borderWidth` 数据往返、单格红色粗框导出渲染和分页 fragment 顶边框渲染。
