...
- 图片参与行内布局测量的规则放在 `layout/`，正文公式控件归属 `formula/`。
...
| `particle/latex/LaTexParticle.ts` | `convertLaTextToSVG()` / `render()` | LaTeX 转图片，供公式菜单和历史预览链路使用。 | image particle |
