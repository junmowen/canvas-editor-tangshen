...
export interface Expr {
  ...
  /** 模式标识，用于选择当前处理分支。 */
  mode: string
  /** LaTeX 解析子节点，用于保留公式树的嵌套结构。 */
  chld: Expr[]
  bbox: Bbox
}

/** 解析atom，把源码文本转换为内部结构。 */
export function parseAtom(x: string): Expr {
  return {
...
export function environments(exprs: Expr[]) {
  let i = 0
...
/** 计算 Bbox 对应的布局或状态。 */
export function computeBbox(exprs: Expr[]): Bbox {
  let xmin = Infinity
  let xmax = -Infinity
  let ymin = Infinity
  let ymax = -Infinity
...
export function group(exprs: Expr[]): Expr {
  if (!exprs.length) {
    // @ts-ignore
    return null
  }
...
export function align(exprs: Expr[], alignment = 'center'): void {
  for (let i = 0; i < exprs.length; i++) {
...
export function plan(expr: Expr, mode = 'math'): void {
  const tmd: string =
    {
      '\\text': 'text',
      '\\mathnormal': 'math',
...
export function flatten(expr: Expr) {
...
export function render(expr: Expr): number[][][] {
  // 初始化 o 列表。
  const o: number[][][] = []
  for (let i = 0; i < expr.chld.length; i++) {
