...
/** 图片预览位图缓存item契约，用于约束内部流程中传递的数据结构。 */
...
/** 图片预览位图缓存stats契约，用于约束内部流程中传递的数据结构。 */
export interface IImagePreviewBitmapCacheStats {
  /** 当前缓存数量，用于衡量资源池占用。 */
  count: number
  /** 最大缓存数量，用于限制资源池规模。 */
  maxCount: number
  /** 缓存写入次数，用于统计资源加入缓存的频率。 */
  setCount: number
  /** 缓存命中次数，用于统计可复用资源的命中情况。 */
  hitCount: number
  /** 缓存未命中次数，用于评估缓存缺口。 */
  missCount: number
  /** 缓存命中率，用于评估图片或位图缓存效果。 */
...
export class ImageParticle {
  ...
  /** 处理dragfloat图片事件，衔接指针交互和编辑器状态更新。 */
  ...
  /** 销毁destroyfloat图片相关资源，解除事件监听并释放持有对象。 */
