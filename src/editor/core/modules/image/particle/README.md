...
`image/particle/` 存放图片业务元素的绘制、预览和公式菜单预览能力。
...
| `latex/` | LaTeX 转 SVG，供公式菜单和历史预览链路使用 |
...
| `latex/LaTexParticle.ts` | `convertLaTextToSVG(laTex)` / `render(...)` | 将 LaTeX 转为 SVG，供公式菜单和历史预览链路使用。 | 公式菜单预览、历史图片化渲染 |
