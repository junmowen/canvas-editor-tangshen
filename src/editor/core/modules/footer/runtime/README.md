...
| 文件 | 职责 |
| --- | --- |
| `Footer.ts` | 页脚作用域元素列表、行布局、position 计算和页脚绘制 |

## 函数说明

| 文件 | 函数 / 方法 | 作用 | 调用位置 |
| --- | --- | --- | --- |
| `Footer.ts` | `getRowList(pageNo)` / `getElementList(pageNo)` / `getPositionList(pageNo)` | 按页读取页脚行、元素和 position。 | `draw/data/DrawDataAccess.ts`、`draw/track-change/TrackChangeService.ts` |
| `Footer.ts` | `setPageScopes(pageScopes)` / `ensureElementList(pageNo)` | 替换或懒创建页脚作用域元素列表。 | 页面数据设置、区域编辑和运行时恢复链路 |
| `Footer.ts` | `compute()` | 计算页脚作用域行布局。 | 页脚数据变更链路 |
