...
function resolveTableAbsoluteCursorContext(
  ...
  const accessor = draw.getServices().tableLayoutSnapshotAccessor
  const table = draw
...
function setPagedTableCursor(editor: Editor, tableId: string, index: number) {
  ...
  const canvas = document.querySelector(`canvas[data-index="${pageNo}"]`) as HTMLCanvasElement | null
  expect(canvas).to.not.eq(null)
  const targetCanvas = canvas!
  const rect = targetCanvas.getBoundingClientRect()
  const evt = {
    target: targetCanvas,
    currentTarget: targetCanvas,
    offsetX: x,
    offsetY: y,
    clientX: rect.left + x,
    clientY: rect.top + y,
