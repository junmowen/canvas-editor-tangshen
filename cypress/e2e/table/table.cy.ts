...
function flushScheduledFrameRender(editor: Editor) {
  const draw = (editor as any).draw
  draw.getServices().renderInvalidationManager.flushScheduledFrameRender()
}

function getTableLayoutSnapshotAccessor(editor: Editor) {
  return (editor as any).draw.getServices().tableLayoutSnapshotAccessor
}
...
describe('菜单-表格', () => {
  ...
  it('按内容自动调整表格列宽', () => {
    cy.getEditor().then((editor: Editor) => {
      ...
      flushScheduledFrameRender(editor)
      const beforeBounds = getTableLayoutSnapshotAccessor(editor)
      ...
      const afterBounds = getTableLayoutSnapshotAccessor(editor)
        .getFragmentCellBounds(table.id)
        .find((item: any) => item.trIndex === 0 && item.tdIndex === 1)
  ...
  it('閫夋嫨鏁翠釜琛ㄦ牸鏃朵細楂樹寒', () => {
    cy.getEditor().then((editor: Editor) => {
      ...
      editor.command.executeTableSelectAll()
      flushScheduledFrameRender(editor)
      const firstCellBounds = getTableLayoutSnapshotAccessor(editor)
        .getFragmentCellBounds(table!.id)
