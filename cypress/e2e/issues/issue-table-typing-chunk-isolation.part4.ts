...
describe('表格输入原分页正确性基线', () => {
  ...
  it('clears stale table borders when a third-page table is pushed to the fourth page', () => {
    const appendText = '继续输入正文把第三页表格推到第四页。'.repeat(30)
    let tableId = ''
    let prefixLength = 0
    ...
    cy.getEditor().then((editor: Editor) => {
      const prepared = prepareThirdPageTableMoveDocument(editor)
      ...
      expect(oldBorderBox, '旧第三页表格边框采样区域').to.not.eq(null)
      expect(oldBorderBox!.pageNo, '表格初始应在第三页').to.eq(2)
    ...
    cy.document().then(doc => {
      ...
      expect(oldMaxDarkRun, '输入前第三页表格边框必须可见').to.be.greaterThan(20)
    })
    ...
    cy.getEditor().then((editor: Editor) => {
      ...
      expect(nextBorderBox!.pageNo, '表格应被推到后续页').to.be.greaterThan(
        oldBorderBox!.pageNo
      )
      expect(stats.chunkLayout.patchSuccessCount, '正文推动第三页表格应命中页级父子窗口').to.be.greaterThan(0)
      expect(
        stats.chunkLayout.dirtyRangePlanCount,
        '正文推动第三页表格应记录 dirty range planner 输出'
      ).to.be.greaterThan(0)
      expect(
        stats.chunkLayout.dirtyRangeMissActualCount,
        `dirty range planner 不能漏掉第三页表格迁移影响页：${JSON.stringify(stats.chunkLayout)}`
      ).to.eq(0)
      expect(
        stats.chunkLayout.dirtyRangeLastIncludesTableRange,
        '第三页表格迁移时 dirty range 必须命中表格范围'
      ).to.eq(true)
      expect(
        countTableRowsIntersectingBox(editor, oldBorderBox!),
        '第三页旧表格区域不能继续存在表格 fragment 行'
      ).to.eq(0)
    })
    ...
    cy.document().then(doc => {
      ...
      expect(
        nextOldMaxDarkRun,
        `第三页旧表格边框连续线段必须消失，输入前 ${oldMaxDarkRun}`
